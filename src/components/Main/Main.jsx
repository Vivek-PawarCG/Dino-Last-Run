...
const LandscapePrompt = () => {
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-black flex flex-col justify-center items-center text-white text-center p-8 font-pixel">
      ...
      <p className="text-xs leading-6 text-gray-400">Dino: Last Run requires Landscape mode to sprint and enjoy the scenery!</p>
    </div>
  );
};

const MainMenu = ({ onStart, onDiary, personality, setPersonality }) => (
  <div className="text-white flex flex-col items-center gap-8 w-full h-full bg-black justify-center absolute inset-0">
    <h1 className="md:text-5xl text-3xl text-biome-jungle-fg animate-pulse pb-4 text-center">DINO: LAST RUN</h1>
    
    <div className="flex flex-wrap justify-center gap-4 md:text-sm text-[10px] font-pixel">
      <button onClick={() => setPersonality('STOIC REX')} className={personality === 'STOIC REX' ? 'text-biome-final-fg' : 'text-gray-500'}>STOIC</button>
      <button onClick={() => setPersonality('CHATTY REX')} className={personality === 'CHATTY REX' ? 'text-biome-final-fg' : 'text-gray-500'}>CHATTY</button>
      <button onClick={() => setPersonality('EXISTENTIAL')} className={personality === 'EXISTENTIAL' ? 'text-biome-final-fg' : 'text-gray-500'}>EXISTENTIAL</button>
    </div>

    <div className="flex flex-wrap justify-center gap-6 mt-6 font-pixel">
      <button onClick={onStart} className="px-8 py-4 bg-white text-black hover:bg-biome-final-fg hover:text-white transition-colors text-sm md:text-lg">START LOOP</button>
      <button onClick={onDiary} className="px-8 py-4 border-2 border-white hover:text-biome-final-fg hover:border-biome-final-fg transition-colors text-sm md:text-lg">DIARY</button>
    </div>
  </div>
);
...
const DeathScreen = ({ stats, onRestart, onDiary }) => {
  ...
  return (
    <div className="text-white flex flex-col items-center justify-center gap-6 w-full h-full bg-black/95 p-8 shadow-2xl relative z-20 absolute inset-0">
...
export default function Main() {
  ...
  const [personality, setPersonality] = useState('STOIC REX');
